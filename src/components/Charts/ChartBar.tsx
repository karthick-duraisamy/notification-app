...
const ChartBar = ({ data }: IChartBar) => {
  ...
  data.forEach((value: SettingsWiseUsage) => {
    if (value.email_instances !== 0) {
      xaxisValue = xaxisValue < value.email_instances ? value.email_instances : xaxisValue;
      chartData[chartData.length] = {
        ...value,
        total_api_request_data: value.total_api_request !== 0 ? value.total_api_request : 2000,
  ...
  if (xaxisValue < 100) {
    ...
    const remainderValue = xAxisCalcValue - (xaxisOneFourthValue % xAxisCalcValue);
    ...
    const xaxisMultipleValue = remainderLength === 3 ? 100 : remainderLength === 4 ? 1000 : 10000;
    xaxisValue =
  ...
  const renderCustomizedLabel = (props: any) => {
    ...
    return (
      <g>
        <text x={xAxis + width / 2} y={yAxis - radius} fill="#000" textAnchor="middle" dominantBaseline="middle">
  ...
  return (
    ...
    <>
      {chartData.length > 0 ? (
        <ResponsiveContainer className="cls-chartbar" width="100%" height={300} data-testid="chartbar">
          <BarChart data-testid="barChart" width={50} height={50} data={chartData}>
            ...
            <Bar barSize={13} dataKey="Notsent" stackId="a" fill="#FF6363">
              <LabelList className="cls-chartbar-datalabel" content={renderCustomizedLabel} />
            </Bar>
          </BarChart>
        </ResponsiveContainer>
      ) : (
