...
const PieChartExample = ({ data }: IPieChartExample) => {
    ...
    return (
      <g>
        <circle
          ...
          fill="#ffffff"
        ></circle>
        <text x={x} y={y - 5} fill="#000000" textAnchor="middle" dominantBaseline="central">
          {`${(percent * 100).toFixed(0)}%`}
        </text>
        <text x={x} y={y + 10} fontSize={11} fill="#636884" textAnchor="middle" dominantBaseline="central">
          {name}
        </text>
      </g>
    );
  ...
  const InnerLabel = () => {
    return (
      <g>
        <text x="50%" y="50%" fill="#3d405c" className="recharts-text recharts-label" textAnchor="middle">
          <tspan alignmentBaseline="middle" fontSize="26">
            {data.reduce((total: any, val: any) => val.value + total, 0)}
          </tspan>
        </text>
        <text x="50%" y="60%" fill="#3d405c" className="recharts-text recharts-label" textAnchor="middle">
  ...
  return (
    ...
    <>
      {data[0]['value'] !== 0 || data[1]['value'] !== 0 ? (
        <ResponsiveContainer width="100%" height={350} data-testid="piechart">
