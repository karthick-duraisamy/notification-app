...
import { FilterIconAnimation } from '@/components/AnimationsExport/AnimationsExport';
...
const Project = () => {
  ...
  const { isSuccess: _masterSuccess, data: masterData, isFetching, isLoading } = useGetProjectMasterInfoQuery({});
  ...
  const [filterShowUp, setFilterShowUp] =useState<boolean>(false);
  ...
  const columns: TableProps<DefaultRecordType>['columns'] = [
    ...
    },
    {
      ...
      render: (_projectId: string | number, data: DefaultRecordType) => (
        <>
...

    {
      ...
      className: 'popover',
      responsive: ['lg', 'xs', 'sm'],
      render: (_projectId: string | number, data: DefaultRecordType) => <ProjectActions data={data} />
    }
  ];
  ...
  return (
    <>
      ...
      <Row className='cls-common-style'>
        <Col span={24}>
          <Row justify="space-between" className="cls-project-detail cls-border-bottom" align="middle" gutter={[0, 24]}>
        <Col flex="auto">
        ...
        <Col className='cls-custom-filter-btn'>
          <Button icon={<FilterIconAnimation />} className='cls-apply-btn cls-hover-expand' size="middle" onClick={() => { setFilterShowUp(!filterShowUp) }}>
            {/* <span className="cls-button-text">Filters</span> */}
          </Button>
        </Col>
      </Row>
      {filterShowUp && <Row style={{ margin: '0px 0px 0px 0px' }}>
        <Col span={24} className="cls-project-filter cls-filter-ele cls-border-bottom">
        ...
        <Col span={24} className="cls-table">
          {isLoading || isFetching ? (
            ...
            <CustomTable
                ...
                sortableColumns={[ 'title', 'created_by', 'actions']}
              />
          )}
        </Col>
      </Row>
