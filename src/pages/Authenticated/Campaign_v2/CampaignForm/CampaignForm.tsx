...
import { AIicon, CustomEditIcon } from "@/components/Icons/Icons";
import {
  formatMinutes,
  generateRange,
  validateNoSQL,
...
const CampaignForm = () => {
  ...
  const [completeTemplateData, setCompleteTemplateData] =
  ...
  const [campaignMasterInfo, campaignMasterInfoStatus] =
    useLazyGetCampaignMasterInfoQuery();
  const [getFolderInfo, getFolderInfoStatus] = useLazyGetFolderQuery();
  const [getTemplateInfo, getTemplateInfoStatus] =
    useLazyGetTemplateForCampaignQuery();
  const [getGroupsData, getGroupsDataStatus] = useLazyGetGroupsListQuery();
  const [getTemplateService, getTemplateServiceStatus] =
  ...
  useEffect(() => {
    if (
      (campaignMasterInfoStatus.isSuccess &&
        "data" in campaignMasterInfoStatus &&
        campaignMasterInfoStatus.data) ||
      (getFolderInfoStatus.isSuccess &&
        "data" in getFolderInfoStatus &&
        getFolderInfoStatus.data)
    ) {
  ...
  useEffect(() => {
    project !== undefined &&
      project !== null &&
      campaignMasterInfo({ project: project });
  }, []);
  ...
  useEffect(() => {
    getTemplateServiceStatus?.isSuccess &&
      setCompleteTemplateData(
        (getTemplateServiceStatus as any)?.data?.response?.data
      );
      ...
      if (
        ...
        let errorObj =
          response?.response?.errors &&
            ...
            : undefined;
        let errorMsg =
          errorObj && response?.response?.errors
            ? response.response.errors[errorObj]?.[0]
            : "Invalid input";
  ...
  const editSettings = () => {
    console.log(formDetails?.selectedSetting);
  };

  // Create campaign Batch form component
  const LaunchTypeBatch = () => (
    <Row
      className="cls-selected-field cls-manual-select"
  ...
  const LaunchTypeBatchTimeZone = () => (
    <Row
      className="cls-selected-fields cls-manual-select"
    ...
    <Row className="cls-selected-fields">
      <Col span={24}>
        ...
        <Row>
          <Col xs={24} sm={8} md={8} xl={4} xxl={4}>
            ...
            >
              <FolderFilter
                ...
                activeFolder={topicValSelect}
                handler={topicValueSet}
              />
            </Form.Item>
          </Col>
          ...
          <Col xs={24} sm={8} md={8} xl={4} xxl={4}>
            ...
            >
              <Select placeholder="Select connection type">
                {(
                  campaignMasterInfoStatus as any
                )?.data?.response?.data?.campaign_types?.map((option: any) => (
                  <Option
                    key={option?.value}
                    value={option?.value}
                    data-testid="optionBox"
                  >
                    {option.label}
                  </Option>
                ))}
          ...
          <Col xs={24} sm={24} md={24} xl={8} xxl={8} className="cls-align-end">
            {formDetails?.selectedSetting && (
              // <div className="cls-setting-details">
              <Row className="cls-setting-details">
                <Col span={12} className="cls-email-info">
                  <label>From Email: </label>
                  <span>{formDetails.selectedSetting.from_email_id}</span>
                </Col>
                <Col span={11} className="cls-email-info">
                  <label>Reply Email: </label>
                  <span>{formDetails.selectedSetting.from_email_id}</span>
                </Col>
                <Col
                  span={1}
                  className="cls-edit-btn cls-icon-bounce"
                  onClick={() => {
                    editSettings();
                  }}
                >
        ...
        <Row>
          <Col xs={24} sm={8} md={8} xl={8} xxl={8}>
            ...
            >
              <Select
                ...
                onChange={(key: any) => templateFieldReset(key)}
              >
                {(getFolderInfoStatus as any)?.data?.response?.data?.map(
                  (option: any) => (
          ...
          >
            ...
            >
              ...
              >
                {(getTemplateInfoStatus as any)?.data?.response?.data?.map(
                  (option: any) => (
                    <Option
    ...
    <Row className="cls-selected-fields">
      <Col span={24}>
        <Row>
          ...
          {getGroupsDataStatus?.isSuccess &&
            formDetails?.contactType === "group" && (
              <Col xs={24} sm={8} md={8} xl={8} xxl={8}>
                <Form.Item
                ...
                >
                  <GroupSelect
                    ...
                    folders={extractGroupData(
                      (getGroupsDataStatus as any)?.data?.response?.data
                        ?.results
                    )}
                    handler={(groupIds: any, counts: any) => {
                      // Get the selected group names for display
                      const groupFolders = extractGroupData(
                        (getGroupsDataStatus as any)?.data?.response?.data
                          ?.results
                      );
                      const selectedGroupNames = groupFolders
                        .filter((folder: any) =>
                          groupIds.includes(folder.group_id)
                        )
        ...
        >
          <Row>
            <Col className="cls-form-fields-select" span={24}>
              {(
                campaignMasterInfoStatus as any
              )?.data?.response?.data?.launch_options?.map(
                (option: any, index: any) => (
                  <span
                    className={
                      index == (formDetails as any)?.scheduleIndex
                        ? "cls-active"
                        : ""
                    }
                    ...
                    onClick={() => {
                      setFormDetails((prev: any) => ({
                        ...prev,
                        scheduleTypeType: option,
                        scheduleIndex: index,
                        launch_option: (formDetails as any)?.scheduleTypeType
                          ?.value,
                      }));
                    }}
                  >
            ...
            {((formDetails as any)?.scheduleTypeType?.label ===
  ...
  const checkSectionCompletion = (section: string) => {
    switch (section) {
      ...
      case "template":
        return (
          form.getFieldValue("r_template_id") &&
          form.getFieldValue("r_setting_id")
        );
      case "contact":
        return (
          form.getFieldValue("contact_type") &&
          form.getFieldValue("r_contact_group")?.[0]
        );
      case "schedule":
  ...
  const scrollToPanel = (panelKey: string) => {
    ...
    if (panel) {
      ...
      if (layout) {
        ...
        const scrollTop =
          layout.scrollTop +
          (layoutRect.height - panelRect.height) -
          headerOffset;
        ...
        <Col span={24}>
              ...
              <Collapse
                ...
                onChange={(keys) => {
                  if (Array.isArray(keys)) {
                    ...
                    setActiveKeys((prev) => {
                      const newKeys = [...prev, keys].filter(
                  ...
                  header={
                    <Row className="cls-panel-header" justify={"space-between"}>
                      <Col md={17} xl={19}>
                        <AimOutlined
                          style={{ color: "#1890ff" }}
                          className="cls-icon-bounce cls-padding-right"
                        />{" "}
                        Campaign basic
                      </Col>
                      <Col
                        className="cls-icon-bounce"
                        style={{
                          color: `${completedSections.campaign
                              ? "var(--btn-apply-color)"
                  ...
                  header={
                    <Row className="cls-panel-header" justify={"space-between"}>
                      ...
                      <Col
                        className="cls-icon-bounce"
                        style={{
                          color: `${completedSections.template
                              ? "var(--btn-apply-color)"
                              : "var(--btn-primary-color)"
                            }`,
                        }}
                ...
                <Panel
                  header={
                    <Row className="cls-panel-header" justify={"space-between"}>
                      <Col md={17} xl={19}>
                        <TeamOutlined
                          style={{ color: "var(--btn-apply-color)" }}
                          className="cls-icon-bounce cls-padding-right"
                        />{" "}
                        Contact information
                      </Col>
                      ...
                      <Col
                        className="cls-icon-bounce"
                        style={{
                          color: `${completedSections.contact
                              ? "var(--btn-apply-color)"
                              : "var(--btn-primary-color)"
                            }`,
                        }}
                  ...
                  header={
                    <Row className="cls-panel-header" justify={"space-between"}>
                      ...
                      <Col
                        className="cls-icon-bounce"
                        style={{
                          color: `${completedSections.schedule
                              ? "var(--btn-apply-color)"
                              : "var(--btn-primary-color)"
                            }`,
                        }}
          ...
          {completeTemplateData && (
            <Modal
              ...
              closeIcon={
                <CloseCircleOutlined
                  style={{ fontSize: 22, color: "#FF4646" }}
                />
              }
              closable={true}
              title={null}
          ...
          >
            ...
            {templateModal.group && (
              <ManageGroupForm
                ...
                groupData={undefined}
                closeModal={() => () =>
                  setTemplateModal((prev: any) => ({ ...prev, group: false }))}
              />
            )}
          </Modal>
        </Col>
